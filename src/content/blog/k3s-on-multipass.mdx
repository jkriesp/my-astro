...
<CodeBlock
  code={`multipass find`}
  lang="bash"
  title="List available Multipass images"
/>

<CodeBlock
  code={`Image                       Aliases           Version          Description
...
<CodeBlock code={`multipass list`} lang="bash" title="Check VM status" />
...
<CodeBlock
  code={`multipass exec k3s-control -- bash -c "curl -sfL https://get.k3s.io | sh -"`}
  lang="bash"
  title="Install k3s on control node"
/>
...
wait`}
  lang="bash"
  title="Worker node setup script"
/>

Run the script with IP and token:

<CodeBlock
  code={`./install_k3s_workers.sh $IP $TOKEN`}
  lang="bash"
...
<CodeBlock
  code={`multipass exec k3s-control -- bash
